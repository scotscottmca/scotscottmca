...
| `scripts/usage-stats.mjs` | Builds that JSON from `~/.claude/projects` transcripts + `gh` and pushes it to a gist. Cuts every figure by all three windows; `USAGE_CYCLE_DAY` (default 1) is the subscription's renewal day and anchors the cycle. Run by launchd every 15 min via `scripts/com.scotscottmca.usage.plist` (see comments in the plist to install). `--selftest` exercises the cross-device merge. |
