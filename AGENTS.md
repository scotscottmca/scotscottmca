...
| `src/pages/usage.astro` | Unlisted `/usage` page (noindex, out of the sitemap, not in nav): live Claude sessions and Claude/GitHub figures over three windows — 7 days, 14 days, and the billing cycle so far — switched client-side from one fetch of `SITE.usageDataUrl`. |
